...
contract RebaseTokenTest is Test {
    ...
    function setUp() public {
        // 1. Deploy RebaseToken as owner
        vm.startPrank(owner);
        rebaseToken = new RebaseToken();

        // 2. Deploy Vault pointing to that token
        vault = new Vault(IRebaseToken(address(rebaseToken)));

        // 3. Give the Vault the right to mint & burn
    ...
    function testRedeemStraightAway1(uint256 amount) public {
        amount = bound(amount, 1e5, type(uint96).max);
        // Deposit funds
        vm.startPrank(user);
        vm.deal(user, amount);
    ...
    function testCannotCallMint() public {
        ...
        vm.startPrank(owner);
        vm.expectRevert();
        rebaseToken.mint(owner, SEND_VALUE, interestRate);
        vm.stopPrank();
    }
    ...
    function testCannotCallBurn() public {
        ...
        vm.startPrank(owner);
        vm.expectRevert();
        rebaseToken.burn(owner, SEND_VALUE);
        vm.stopPrank();
    }
    ...
    function testDeposit(uint256 amount) public {
        ...
        uint256 initialUserTokenBalance = rebaseToken.balanceOf(user);
        ...
        uint256 finalUserTokenBalance = rebaseToken.balanceOf(user);

        // Assertions
        assertEq(
            finalVaultBalance,
            initialVaultBalance + amount,
            "Vault ETH balance should increase"
        );
        assertEq(
            finalUserTokenBalance,
            initialUserTokenBalance + amount,
            "User should receive tokens equal to amount deposited"
        );
    }
